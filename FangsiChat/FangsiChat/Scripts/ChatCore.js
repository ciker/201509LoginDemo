...
; !function (win, undefined) {
    ...
    ChatCore = {},
    systemHub = $.connection.systemHub,
    onlinenum = 0;//在线人数
    ...
    ChatCore.popchatbox = function (othis) {
        var node = ChatCore.node, userId = othis.attr('id'), dataId = othis.attr('data-id'), param = {
            ...
            href: 'http://www.fangsi.net/'//config.hosts + 'user/' + dataId //用户主页
    ...
    ChatCore.event = function () {
        ...
        systemHub.client.onConnected = function (id, userName, allUsers) {
            ...
            if (onlinenum > 0) {
                ...
                for (; i < onlinenum; i++) {
                    str += '<li id="userid-' + allUsers[i].UserID + '" data-id="' + allUsers[i].ConnectionId + '" class="ChatCore_childnode" type="one"><img src="/Content/Images/avatar.jpg?' + allUsers[i].UserID + '"  class="ChatCore_oneface"><span  class="ChatCore_onename">' + allUsers[i].UserName + '(' + allUsers[i].DeptName + ')</span><em class="ChatCore_time">' + allUsers[i].LoginTime + '</em></li>';
                }
                str += '</ul></li>';
                myf.html(str);
            } else {
        ...
        systemHub.client.onNewUserConnected = function (id, userID, userName, deptName, loginTime) {
            ...
            str += '<li id="userid-' + userID + '" data-id="' + id + '" class="ChatCore_childnode" type="one"><img src="/Content/Images/avatar.jpg?'+ userID + '"  class="ChatCore_oneface"><span  class="ChatCore_onename">' + userName + '(' + deptName + ')</span><em class="ChatCore_time">' + loginTime + '</em></li>';
            myf.append(str);
        };
